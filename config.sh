...
#############################
# Confirm GCP configuration #
#############################
function check_gcp {
  echo -e "${LPURP}***** Confirm GCP Configuration *****${NC}"
  MY_PATH=`eval echo "~$SUDO_USER"`
  echo -e "${LPURP}"
  echo "Don't forget to run gcloud init!"
  echo -e "${NC}"
}
...
function check_terra_config {
  ...
  if [ ! -f ${PWD}/gcp/terraform.tfvars ] ; then
    echo -e "${YELLOW}"
    echo "${PWD}/gcp/terraform.tfvars not found!"
    echo "Google Cloud won't work right."
    ...
    echo -e "${CYAN}"
    echo "Found tfvars file in ${PWD}/gcp/terraform.tfvars"
    echo -e "${NC}"
  fi
}
...
function config_apple {
  ...
  if which brew ; then
    ...
    if ! which gcloud ; then
      echo -e "${CYAN}"
      echo "install gcloud"
      curl -L -# -o ~/google-cloud-sdk-288.0.0-darwin-x86_64.tar.gz \
      'https://dl.google.com/dl/cloudsdk/channels/rapid/downloads/google-cloud-sdk-288.0.0-darwin-x86_64.tar.gz' 
      gzip -d ~/google-cloud-sdk-288.0.0-darwin-x86_64.tar.gz
      tar xf ~/google-cloud-sdk-288.0.0-darwin-x86_64.tar
      ~/google-cloud-sdk/install.sh
      rm ~/google-cloud-sdk-288.0.0-darwin-x86_64.tar
